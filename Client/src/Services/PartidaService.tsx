...
export const PartidaService = {
  GetMatches: () =>
    HandleTry<Partida[]>(async () => {
      const res = await fetch("http://192.168.0.101:3000/frc/matches");
  ...
  GetTeams: () =>
    HandleTry(async () => {
      const res = await fetch("http://192.168.0.101:3000/frc/teams");
  ...
  AddMatch: (payload: any) => {
    return HandleTry(async () => {
      const res = await fetch("http://192.168.0.101:3000/frc/match/new", {
        method: "POST",
  ...
  EndJudgeMatch: (id: number, alianca: any) => {
    return HandleTry(async () => {
      ...
      const res = await fetch(
        `http://192.168.0.101:3000/frc/match/judge/end/${id}`,
        {
          method: "PATCH",
          headers: {
  ...
  DeleteMatch: (id: number) => {
    return HandleTry(async () => {
      const res = await fetch(`http://192.168.0.101:3000/frc/match/delete/${id}`, {
        method: "DELETE",
      });
  ...
  GetAlliencesByMatch: (id: number) =>
    HandleTry<Aliança[]>(async () => {
      const res = await fetch(
        `http://192.168.0.101:3000/frc/match/${id}/alliances`
      );
  ...
  GetMatchInfo: (id: number) => {
    return HandleTry(async () => {
      const res = await fetch(`http://192.168.0.101:3000/frc/match/${id}/score`);
